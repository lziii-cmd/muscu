...
| 2026-08-27 | Brouillon local de séance, écrit à chaque frappe | La saisie ne vivait que dans l'état React : quitter l'écran ou laisser iOS décharger la page suffisait à tout perdre, à 23h en salle | Sauvegarde au seul bouton « Terminer » |
| 2026-08-27 | Attendre l'envoi avant de recharger la journée | Le parent rechargeait depuis le serveur avant que la mutation n'y soit arrivée, et réaffichait l'état d'avant — ce qui se lit comme « rien n'a été enregistré » | Rechargement immédiat |
| 2026-08-27 | L'état « déjà enregistrée » vient du serveur, pas du seul montage | Revenir sur une séance enregistrée rouvrait le formulaire ; les champs pré-remplis par le programme ressemblent à s'y méprendre à une saisie perdue | État local uniquement |
| 2026-08-27 | Confirmation avant « Séance manquée » quand quelque chose est saisi | Le geste efface les lignes côté serveur et le bouton voisine avec « Terminer la séance » | Suppression silencieuse |
...
| 2026-09-18 | Type de charge **barre / machine / haltères / PDC**, saisie en **kg ou lb** | 40 kg barre, 40 kg machine et 40 kg par haltère ne se comparent pas. La base stocke des kilos, `weight_unit` garde l'unité lue sur la machine pour la réafficher (migration 0006) | Unité unique « barre / machine » |
| 2026-09-18 | Charge proposée = **dernière séance en salle passée par la double progression**, sinon charge habituelle (`exercise_loads`), sinon programme ; la plus récente de l'habitude et de la séance gagne | Le programme ne doit pas rester figé : une séance au haut de fourchette fait monter la proposition suivante (+2,5/+5 kg, +5/+10 lb), sinon une rep de plus. Règles dans `src/lib/domain/loads.ts`, testées | Valeurs du document seules |
| 2026-09-18 | Un toucher = **une série** ; repos lancé après chaque série | Demande explicite ; la séance se suit série par série, et le repos entre séries est celui qui dérape | Case unique par exercice |
| 2026-09-18 | Saisie après coup **neutre** | Les charges saisies en retard sont bonnes (dit par l'utilisateur) : plus de message « peu fiables » ; seules durée et pauses non chronométrées restent hors des analyses de temps | Badge d'avertissement |
| 2026-09-18 | Calisthénie v2 en document autonome, créneau `matin`, **sans échelles** ; onglet ouvert par le programme | Le document v2 n'a plus d'échelles ; les exiger cachait l'onglet. Point de départ enregistré comme test au premier jour | Onglet conditionné aux échelles |
| 2026-09-18 | Seed : **re-liaison** des séances faites au programme réimporté | Réimporter (ajout de la calisthénie) détachait la séance du 17 ; l'écran ne retrouvait plus ses lignes | Réimport destructif des liens |
| 2026-09-18 | Diète : journée type = **mardi semaine A** de la v2 ; catalogue d'aliments enrichi, rien retiré | Le document v2 est partiel (renvoie à un original absent) ; le catalogue est partagé avec Nourah | Réécrire la diète entière |
...
| 2026-08-27 | **« Je finis d'enregistrer, je reviens, c'est comme si je n'avais rien enregistré »** | Trois causes distinctes : la saisie n'existait qu'en mémoire jusqu'au bouton final ; la journée était rechargée depuis le serveur avant que la mutation n'y arrive ; et une séance déjà enregistrée rouvrait un formulaire pré-rempli par le programme, impossible à distinguer d'une saisie perdue | Brouillon local écrit à chaque frappe et relu au montage, envoi attendu avant rechargement, carte « Séance enregistrée » restituée depuis le serveur |
...
- **Une saisie n'est acquise qu'après « Terminer la séance ».** Le brouillon local la protège d'une fermeture d'écran, mais il reste local : c'est le serveur qui fait foi.
- **« Séance manquée » efface les exercices**, côté serveur comme en local. C'est voulu — une séance ratée n'a pas d'exercices réalisés — mais irréversible, d'où la confirmation.
